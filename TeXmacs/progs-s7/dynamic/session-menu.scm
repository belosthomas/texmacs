...
(tm-menu (focus-extra-icons t)
  (:require (field-context? t))
  (glue #f #f 8 0)
  (=> (balloon (icon "tm_plugin_input") "Input options")
      (link session-input-menu))
  (=> (balloon (icon "tm_plugin_output") "Output options")
  ...
  (if (!= (get-env "prog-language") "scheme")
      ((balloon (icon "tm_stop") "Interrupt execution")
       (plugin-interrupt))
      ((balloon (icon "tm_clsession") "Close session")
       (plugin-stop))))
