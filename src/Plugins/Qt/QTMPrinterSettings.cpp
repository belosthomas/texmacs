...
QTMPrinterSettings::QTMPrinterSettings()
        : collateCopies(true), blackWhite(true), printerName(""), fileName(""),
          ...
          pagesPerSide(1), pagesOrder(LR_TB), orientation(QPageLayout::Orientation::Portrait) {
...
QTMPrinterSettings::getFromQPrinter(const QPrinter &from) {
    ...
    orientation = (from.pageLayout().orientation() == QPageLayout::Orientation::Landscape)
                  ? QPageLayout::Orientation::Landscape : QPageLayout::Orientation::Portrait;
    paperSize = qtPaperSizeToQString(from.pageLayout().pageSize());
    dpi = from.resolution();
...
QTMPrinterSettings::setToQPrinter(QPrinter &to) const {
    ...
    to.pageLayout().setOrientation(
            (orientation == QPageLayout::Orientation::Landscape) ? QPageLayout::Orientation::Landscape
                                                                 : QPageLayout::Orientation::Portrait);
    to.setOutputFileName(fileName);
    to.pageLayout().setPageSize(qStringToQtPaperSize(paperSize));
#if (QT_VERSION >= 0x040700)
 ...
 */
QString
QTMPrinterSettings::qtPaperSizeToQString(const QPageSize &size) {
...
#define PAPER(fmt)  if(_size == "fmt") return QPageSize(QPageSize::fmt)
    PAPER (A0);
    PAPER (A1);
    PAPER (A2);
    PAPER (A3);
    ...
    PAPER (B10);
    PAPER (Letter);
    return QPageSize(QPageSize::A4);  // Default
#undef PAPER
}
...
WinQTMPrinterSettings::availablePrinters() {
    QList<QPair<QString, QString> > _ret;
            ...
            _ret
                    << QPair<QString, QString>(printer.printerName(), printer.printerName());
    return _ret;
}
