...
public:
    ...
    void apply() {
        qDebug() << "qt_key_command_rep::apply";
        if (N(ks)) {
            texmacs::getApplication().withCurrentEditor([&](auto *editor) {
                editor->handle_keypress(ks, texmacs_time());
            });
        }
    }
...
public:
    ...
    void apply() {
        qDebug() << "qt_toggle_command_rep::apply";
        if (qwid) {
            cmd(list_object(object_from(qwid->isChecked())));
        }
    }
...
public:
    ...
    void apply() {
        qDebug() << "qt_enum_command_rep::apply";
        if (qwid)
...
qt_ui_element_rep::get_payload(qt_widget qtw, types check_type) {
    TM_ASSERT (check_type == none || qtw->type == check_type,
...
qt_ui_element_rep::as_qaction() {
    ...
    switch (type) {
            ...
            // a widget with an X pixmap icon
        {
            ...
            act = new QTMAction(NULL);
            act->setIcon(dynamic_cast<texmacs::Application *>(qApp)->pixmapManager().getIcon(
                    QString::fromStdString(std::string(image.data(), N(image)))));
        }
            break;

        default: TM_FAILED (c_string("qt_ui_element: unknown type for as_qaction, "
                                     * type_as_string()));
    }
    ...
    switch (type) {
        ...
        case pullright_button: {
            ...
            if (qtw->type == xpm_widget) {
                ...
                b->setIcon(dynamic_cast<texmacs::Application *>(qApp)->pixmapManager().getIcon(
                        QString::fromStdString(std::string(image.data(), N(image)))));
        ...
        case xpm_widget: {
            ...
            l->setPixmap(dynamic_cast<texmacs::Application *>(qApp)->pixmapManager().getPixmap(
                    QString::fromStdString(std::string(image.data(), N(image))), QSize(64, 64)));
            qwid = l;
        }
            break;
        ...
        case icon_tabs_widget: {
            ...
            for (i = 0; i < N(tabs); i++) {
                ...
                QLabel *label = qobject_cast<QLabel *>(prelabel);
                QWidget *body = concrete(bodies[i])->as_qwidget();
                tw->addTab(body, dynamic_cast<texmacs::Application *>(qApp)->pixmapManager().getIcon(
                        QString::fromStdString(std::string(icons[i].data(), N(icons[i])))), label ? label->text() : "");
