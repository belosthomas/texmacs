...
bool load_string(url u, string &s, bool fatal) {
    ...
    if (!is_rooted_name(r)) {
        qWarning() << "Load String with a non rooted url";
        if (fatal) {
    ...
    QByteArray qdata = qfile.readAll();
    qfile.close();
    s = string(qdata.data(), qdata.size());

    // Cache file contents
    if (N(s) <= 10000 || currently_cached) {
...
bool save_string(url u, string s, bool fatal) {
    if (is_rooted_tmfs(u)) {
    ...
    if (!qfile.open(QIODevice::WriteOnly)) {
        qWarning() << "Can't open file for writing : " << QString::fromStdString(std::string(name.data(), N(name)));
        if (fatal) {
            string err_msg = string("Failed to save file: ") * as_string(u);
            failed_error << err_msg << LF;
            TM_FAILED ("file not writeable");
        }
        return true;
    }
    qfile.write(QByteArray(s.data(), N(s)));
    qfile.close();
